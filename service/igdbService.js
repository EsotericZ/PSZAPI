  ...
  try {
    ...
    for (const gameName of gameNames) {
      ...
      const queryBody = `
        ...
        fields id, name, first_release_date, cover.image_id, aggregated_rating, age_ratings.rating, age_ratings.category, genres.name, storyline, summary, slug;
        where platforms = [167];
        limit 1;
      `;
      ...
      if (data.length > 0) {
        ...
        if (newIGDBGame.rows.length > 0) {
          results.push(newIGDBGame.rows[0]);
        }
      }
    }
...
export const searchGamesByID = async (igdbId) => {
  console.log(`🔍 Searching IGDB for ID: ${igdbId}`);
  ...
  try {
    ...
    if (existingIGDBGame.rows.length > 0) {
      console.log(`✅ Game already in IGDB: ${existingIGDBGame.rows[0].name}`);
      return existingIGDBGame.rows[0];
    }
    ...
    const data = await response.json();
    if (data.length === 0) {
      console.log("❌ No game found in IGDB.");
      return null;
    }
